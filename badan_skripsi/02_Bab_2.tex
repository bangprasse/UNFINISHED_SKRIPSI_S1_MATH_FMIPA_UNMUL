...
\vspace{-4mm}
{\frenchspacing
    Teori graf merupakan kajian matematika dalam lingkup matematika diskret yang relatif baru .
...
{\frenchspacing
    ...
    penentuan rute terpendek sehingga setiap simpul dapat dikunjungi dengan biaya atau waktu minimal \mycite{Bisma:2023}.
    Setiap simpul yang dilewati dalam konsep TSP hanya dilewati tepat satu kali.
    Sehingga fungsi tujuan dari solusi adalah meminimumkan biaya yang diperlukan untuk melewati simpul-simpul tersebut.
    Konsep TSP dapat dimodelkan melalui \textit{integer programming} seperti pada persamaan \ref{pers: model persamaan tsp} berikut.

    \noindent
    Fungsi tujuan
    \begin{equation}
        \label{pers: model persamaan tsp}
        \operatorname{min} (Z) = \sum^{n}_{i=1} \sum^{n}_{j=1} a_{ij}(v_{i}v_{j})
    \end{equation}
    ...
    \begin{equation*}
        \begin{aligned}
             & \sum^{n}_{j=1} a_{ij} = 1,     & i = 1,2,\dots,n \\
             & \sum^{n}_{i=1} a_{ij} = 1,     & j = 1,2,\dots,n \\
    ...
    \begin{table}
        \begin{tabular}{lcp{11cm}}
            $v_{i}v_{j}$ & $ = $ & biaya dari simpul $i$ menuju simpul $j$, dan                                                                             \\
...
{\frenchspacing
    ...
    \begin{enumerate}[align=left, left = 0mm, nolistsep]
        ...
        \item Heuristik Kesederhanaan \par \nobreak
              ...
              Contohnya memutuskan membeli sepatu merek A menggunakan opini yang sedang \textit{trending} di media sosial.
...
\hspace{-0.675cm}dengan $\varepsilon$ adalah faktor \textit{clamping} untuk \textit{velocity}. Nilai $\varepsilon$ berada pada interval nilai acak $\varepsilon \in [0,1]$ dan $X_{max},X_{min}$ adalah batas atas dan bawah dari inisialisasi posisi $(X)$ \mycite{Wang:2022}.
}
...
{\frenchspacing
    ...
    \begin{enumerate}[align = left, left = 0cm, nolistsep]
        ...
        \item Menentukan $P_{best,j}^{(i)}$ setiap partikel dan $G_{best}^{(i)}$. Untuk setiap partikel, $P_{best,j}^{(i)}$ ditentukan dengan menggunakan persamaan berikut:
              ...
              \begin{equation}
                  \label{pers: penentuan pbest}
                  P_{best,j}^{(i)} = \begin{cases} X_{j}^{(i)} & , F(X_{j}^{(i)}) \geq F(P_{best,j}^{(i-1)}) \\ P_{best,j}^{(i-1)} &, F(X_{j}^{(i)}) < F(P_{best,j}^{(i-1)}) \end{cases}
              \end{equation}

              dan untuk $G_{best}^{(i)}$ berdasarkan posisi dengan nilai fungsi \textit{fitness} paling terbaik yang pernah dicapai di antara seluruh partikel pada semua iterasi.
              Posisi terbaik dari setiap partikel di setiap iterasi disimpan ke dalam $P_{best}$.
              Sehingga $G_{best}$ pada iterasi ke-$i$ dapat ditentukan dengan membandingkan nilai fungsi \textit{fitness} dari $P_{best}$ terbaik pada iterasi ke-$i$ dan nilai fungsi \textit{fitness} dari $G_{best}$ pada iterasi sebelumnya.
...
% SubBab
\section{Metode \textit{Min-Max Normalization}}
\vspace{-4mm}
{\frenchspacing
    Metode \textit{Min-Max Normalization} merupakan salah satu metode normalisasi data dengan mengubah ukuran dari rentang data asli ke dalam rentang 0 sampai 1 \mycite{Ambarwari:2019}.
    Normalisasi data dapat mempercepat proses perhitungan data dari suatu analisis.
    Proses normalisasi data dilakukan tanpa mengubah distorsi dari rentang data asli.
    Normalisasi data dengan metode \textit{Min-Max Normalization} dilakukan menggunakan persamaan berikut.

    \begin{equation}
        \label{pers: normalisasi minmax}
